import React, { useState, useEffect, useRef, useCallback } from 'react';
import NextImage from 'next/image';
import { useIntersectionObserver } from '../hooks/useIntersectionObserver';

// 内存缓存，存储已经加载成功的图片URL
const imageCache = new Map<string, boolean>();

interface LazyImageProps extends Omit<React.ImgHTMLAttributes<HTMLImageElement>, 'src' | 'srcSet' | 'loading'> {
  dataSrc: string;
  dataSrcSet?: string;
  placeholder?: React.ReactNode;
  loadingComponent?: React.ReactNode;
  error?: React.ReactNode;
  retryDelay?: number;
  maxRetries?: number;
}

const LazyImage: React.FC<LazyImageProps> = ({
  dataSrc,
  dataSrcSet,
  placeholder = <div className="bg-gray-200 w-full h-full rounded" />,
  loadingComponent = <div className="flex items-center justify-center h-full w-full bg-gray-200 rounded"><div className="loader ease-linear rounded-full border-4 border-t-4 border-blue-500 h-8 w-8 animate-spin"></div></div>,
  error = <div className="flex items-center justify-center h-full w-full bg-gray-200 rounded text-red-500">Error loading image</div>,
  retryDelay = 1000,
  maxRetries = 3,
  ...props
}) => {
  const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');
  const [retryCount, setRetryCount] = useState(0);
  const imgRef = useRef<HTMLImageElement | null>(null);
  const { ref: observerRef, isIntersecting } = useIntersectionObserver({ threshold: 0.1 });
  const loadImageController = useRef<AbortController | null>(null);

  // 加载图片的函数
  const loadImage = useCallback(async () => {
    // 如果图片已经在缓存中，直接设置为成功状态
    if (imageCache.has(dataSrc)) {
      setStatus('success');
      return;
    }

    // 如果重试次数超过最大值，设置为错误状态
    if (retryCount >= maxRetries) {
      setStatus('error');
      return;
    }

    // 设置为加载状态
    setStatus('loading');

    // 创建AbortController来取消请求
    const controller = new AbortController();
    loadImageController.current = controller;

    try {
      // 加载图片
      const img = new Image();
      const promise = new Promise<void>((resolve, reject) => {
        img.onload = () => resolve();
        img.onerror = () => reject(new Error('Image load failed'));
        img.src = dataSrc;
        if (dataSrcSet) {
          img.srcset = dataSrcSet;
        }
      });

      // 等待图片加载完成
      await promise;

      // 将图片URL添加到缓存
      imageCache.set(dataSrc, true);

      // 设置为成功状态
      setStatus('success');
    } catch (err) {
      // 设置为错误状态
      setStatus('error');

      // 增加重试次数
      setRetryCount(prev => prev + 1);

      // 延迟重试
      setTimeout(() => {
        loadImage();
      }, retryDelay * Math.pow(2, retryCount));
    } finally {
      // 清除AbortController
      loadImageController.current = null;
    }
  }, [dataSrc, dataSrcSet, retryCount, maxRetries, retryDelay]);

  // 当元素进入视口时，加载图片
  useEffect(() => {
    if (isIntersecting && status === 'idle') {
      loadImage();
    }
  }, [isIntersecting, status, loadImage]);

  // 组件卸载时，取消所有正在进行的图片加载请求
  useEffect(() => {
    return () => {
      if (loadImageController.current) {
        loadImageController.current.abort();
      }
    };
  }, []);

  // 渲染不同状态的内容
  switch (status) {
    case 'idle':
    case 'loading':
      return (
        <div className="relative" ref={(el) => {
          observerRef(el);
        }} style={{ position: 'relative', width: '100%', height: '100%' }}>
          {status === 'idle' ? placeholder : loadingComponent}
          <NextImage
            src={dataSrc}
            srcSet={dataSrcSet}
            alt={props.alt}
            fill
            style={{ objectFit: 'cover', borderRadius: 'inherit', opacity: 0, transition: 'opacity 0.3s ease-in-out' }}
            onLoad={(e) => {
              const img = e.target as HTMLImageElement;
              img.style.opacity = '1';
            }}
            {...props}
          />
        </div>
      );
    case 'success':
      return (
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
          <NextImage
            src={dataSrc}
            srcSet={dataSrcSet}
            alt={props.alt}
            fill
            style={{ objectFit: 'cover', borderRadius: 'inherit' }}
            {...props}
          />
        </div>
      );
    case 'error':
      return <div className="relative" ref={observerRef}>{error}</div>;
  }
};

export default LazyImage;